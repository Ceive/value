	...
	class HelperArray{
		...
		public static function deleteKeys(array $array,array $keys, $caseLess = false){
			...
			foreach($array as $key => $_){
				if(($caseLess && HelperArray::stringExists($keys,$key,$caseLess)) || !in_array($key,$keys,true)){
					unset($array[$key]);
				}
			}
			return $array;
		...
		/**
		 * @param \Ceive\Util\Named\NamedInterface[] $array
		 * @param $name
		 * @param callable $cmp
		 * @return \Ceive\Util\Named\NamedInterface[]
		 */
		...
		/**
		 * @param NamedInterface[] $array
		 * @param \Ceive\Util\Named\NamedInterface $name
		 * @param callable $cmp
		...
		public static function setNamed(array & $array, NamedInterface $name,callable  $cmp = null, $modeExists = null){
			...
			if(!in_array($modeExists,[null,false,true],true)){
				throw new \LogicException('Mode exists is invalid, must be [null|false|true], passed: "' . HelperString::representFrom($modeExists) . '" ');
			}
			$exists = [];
			$n = $name->getName();
		...
		public static function getCovered(array $array, $uncover = false, $prefix = '',$suffix = '',$uncoveredNumericInteger = false){
			...
			$suffix = (string)$suffix;
			foreach($array as $key => & $item){
				if(HelperString::startWith($prefix,$key) && HelperString::endWith($suffix,$key)){
					if($uncover){
						$key = $prefix?HelperString::trimWordsLeft($key,$prefix):$key;
						$key = $suffix?HelperString::trimWordsRight($key,$suffix):$key;
		...
		public static function valuesStringCase(array $array, $camel = true){
			foreach($array as $key => & $item){
				if(is_string($item)){
					$item = HelperString::camelCase($item,$camel);
				}
			}
			return $array;
		}
		...
		public static function keysCase(array $array, $camel = true){
			...
			foreach($array as $key => & $item){
				if(is_string($key)){
					$newArray[HelperString::camelCase($key,$camel)] = $item;
				}else{
					$newArray[$key] = $item;
				}
			}
		...
		public static function searchString(array $array,$string,$caseLess = false){
			foreach($array as $param => $value){
				if(is_string($value) && HelperString::match($string,$value,$caseLess)){
					return $param;
				}
			}
			return false;
		...
		public static function stringExists(array & $array,$value,$caseLess = false){
			foreach($array as $param => $v){
				if(is_string($value) && HelperString::match($value,$v,$caseLess)){
					return true;
				}
			}
			return false;
		...
		public static function keyCaseExists(array & $array,$key){
			foreach($array as $param => $value){
				if(is_string($key) && HelperString::match($key,$param,true)){
					return true;
				}
			}
			return false;
		...
		public static function orderedKeys(array $array,array $keys, $checkIsset = false, $allowDefault = false, $default = null){
			...
			foreach($keys as $key){
				if(($checkIsset && isset($array[$key])) || (!$checkIsset && array_key_exists($key,$array))){
					...
					throw new \LogicException('HelperArray::orderedKeys Not found key "'.$key.'" in subject array');
				}
			}
			return $a;
		}
