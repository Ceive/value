...
namespace Ceive\Value {
	 ...
	 *
	 */
	class VDateTime extends VNumber{
		...
		public static function timeGetYearHalf($time = null){
			...
			foreach(self::$year_half as  $half => $mNumbers){
				if(HelperArray::stringExists($mNumbers,$mNumber,true)){
					return $half;
				}
			}
			return null;
		...
		public static function timeGetSeason($time = null){
			...
			foreach(self::$seasons as  $season => $months){
				if(HelperArray::stringExists($months,$month,true)){
					return $season;
				}
			}
			return null;
		 ...
		 *               if($auto_amnesty && $this->locked && HelperDateTime::timeIsReached($this->locked_time, $this->locked_delay)){
		 *                   $this->amnesty();
		 *               }
		 *               return $this->locked;
		 *           }
